    ...
    public:
        ...
        void insert_many(std::vector<T> &docs)
        {
            ...
            try
            {
                ...
                if (result)
                {
                    const auto &inserted_ids_view = result->inserted_ids();
                    for (const auto &element : inserted_ids_view)
                    {
                        // The key is the index of the document in the input vector, as a string.
                        size_t index = std::stoul(std::string(element.key()));
                        if (index < docs.size() && element.type() == bsoncxx::type::k_oid)
                        {
                            docs[index]._id = element.get_oid().value;
                        }
                    }
                }
            }
